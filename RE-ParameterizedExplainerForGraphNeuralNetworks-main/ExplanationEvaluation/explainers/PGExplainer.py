...
class PGExplainer(BaseExplainer):
    ...
    def __init__(self, model_to_explain, graphs, features, task, epochs=30, lr=0.003, temp=(5.0, 2.0), reg_coefs=(0.05, 1.0),sample_bias=0):
        ...
        if self.type == "graph":
            self.expl_embedding = self.model_to_explain.embedding_size * 2 #since we are concatenating two together for the embedding put into MLP
        else:
    ...
    def prepare(self, indices=None):
        ...
        """
        # Creation of the explainer_model is done here to make sure that the seed is set; this spits out the latent variables
